...
Matrix Matrix::operator()(slice_t row_range, slice_t column_range)
{
    int res_nrows = row_range.second-row_range.first;
    int res_ncols = column_range.second- column_range.first;

    Matrix res(res_nrows, res_ncols);

    for (int i=row_range.first; i<row_range.second; i++)
    {
        for (int j=column_range.first; j<column_range.second; j++)
        {
            res.data[i-row_range.first][j-column_range.first] = data[i][j];
        }
    }
    return res;
}
